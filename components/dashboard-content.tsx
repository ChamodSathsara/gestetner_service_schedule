  ...
  return (
    <div className="w-full max-w-7xl px-4 md:px-8 py-4 md:py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Welcome back! Here's your service management overview.</p>
      </div>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <CardDescription>Service schedules and breakdowns progress</CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={technicianSummaryData}>
              ...
              />
              <Legend />
              <Line type="monotone" dataKey="services" stroke="#3b82f6" dot={false} name="Service Schedules" />
              <Line type="monotone" dataKey="breakdowns" stroke="#ef4444" dot={false} name="Breakdowns" />
          ...
          <CardContent>
            <div className="space-y-4">
              {technicianPerformance.map((tech) => (
                <div key={tech.id} className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{tech.completedJobs} jobs completed</p>
                    </div>
                  </div>
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent>
            <div className="space-y-3">
              {pendingJobs.map((job) => (
                <div key={job.id} className="p-3 border border-border rounded-lg hover:bg-secondary transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium ${
                        job.jobType === "Service" ? "bg-blue-500/20 text-blue-600" : "bg-red-500/20 text-red-600"
                      }`}
                    >
                      {job.jobType}
                    </span>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <CardDescription>Machine warranty information for all customers</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2 border-border">
                  <th className="text-left p-2 font-semibold text-foreground">Customer Name</th>
                  <th className="text-left p-2 font-semibold text-foreground">Address 1</th>
                  <th className="text-left p-2 font-semibold text-foreground">Address 2</th>
                  <th className="text-left p-2 font-semibold text-foreground">Address 3</th>
                  <th className="text-left p-2 font-semibold text-foreground">Serial No</th>
                  <th className="text-left p-2 font-semibold text-foreground">Machine Description</th>
                  <th className="text-left p-2 font-semibold text-foreground">Ref Code</th>
                  ...
                  <th className="text-left p-2 font-semibold text-foreground">Officer Code</th>
                  <th className="text-left p-2 font-semibold text-foreground">Officer Name</th>
                  <th className="text-left p-2 font-semibold text-foreground">MA Period End</th>
                  <th className="text-left p-2 font-semibold text-foreground">Warranty End</th>
                </tr>
              </thead>
              <tbody>
                {warrantyDetailsData.map((row, idx) => (
                  <tr key={idx} className="border-b border-border hover:bg-secondary transition-colors">
                    ...
                    <td className="p-2 text-foreground">{row.machineRefCode}</td>
                    ...
                    <td className="p-2 text-foreground">{row.mWarrantyEndDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <CardDescription>Machine count summary by technician officer</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2 border-border">
                  ...
                  <th className="text-left p-2 font-semibold text-foreground">Officer Name</th>
                  <th className="text-left p-2 font-semibold text-foreground">Team</th>
                  <th className="text-left p-2 font-semibold text-foreground">Machine Count</th>
                </tr>
              </thead>
              <tbody>
                {warrantySummaryData.map((row, idx) => (
                  <tr key={idx} className="border-b border-border hover:bg-secondary transition-colors">
                    <td className="p-2 text-foreground font-medium">{row.tOfficerCode}</td>
                    <td className="p-2 text-foreground">{row.tOfficerName}</td>
                    <td className="p-2 text-foreground">{row.team}</td>
                    <td className="p-2">
