...
function getEffectiveDate(row: NsRow): string {
...
function isUsingWarrantyDate(row: NsRow): boolean {
...
function getAddress(row: NsRow): string {
...
export default function NsBaseReport() {
  const { GetNSReportData, downloadNSReportExcel, downloadNSReportPdf } =
    useApiConfig();

  // ── State ──────────────────────────────────────────────────────────────────
  const [data, setData] = useState<NsRow[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await GetNSReportData(startDate, lastDate);
  ...
  const handleExcelDownload = async () => {
    setDlLoading("excel");
    try {
      const blob: Blob = await downloadNSReportExcel(startDate, lastDate);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `NS_Report_${startDate}_to_${lastDate}.xlsx`;
      a.click();
  ...
  const handlePdfDownload = async () => {
    setDlLoading("pdf");
    try {
      const blob: Blob = await downloadNSReportPdf(startDate, lastDate);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `NS_Report_${startDate}_to_${lastDate}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, #0f2044 0%, #1a3a6e 60%, #1e4d8c 100%)",
          padding: "18px 28px",
          display: "flex",
      ...
      >
        <div>
          ...
          <div style={{ color: "#90caf9", fontSize: "11px", marginTop: "2px" }}>
            {loading
              ? "Loading…"
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    outline: active
                      ? `2px solid ${z === "ALL" ? "#1a3a6e" : col.color}`
                      : "none",
                  }}
                >
        ...
        {(
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: "13px",
              color: activeTab === tab ? "#1a3a6e" : "#94a3b8",
              borderBottom:
                activeTab === tab
                  ? "3px solid #1a3a6e"
                ...
                {availableYears.map((y) => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontSize: "13px",
                      background: chartYear === y ? "#1a3a6e" : "#f1f5f9",
                          ...
                          <div
                            ...
                            style={{
                              ...
                              background: isHot
                                ...
                                : count > 0
                                  ? "linear-gradient(180deg, #3b82f6 0%, #1a3a6e 100%)"
                                  : "#f1f5f9",
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "6px",
        border: "1px solid #e2e8f0",
        background: active ? "#1a3a6e" : "white",
