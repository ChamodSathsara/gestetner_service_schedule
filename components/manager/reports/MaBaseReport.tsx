...
/** MA always uses maPeriodEnd as the effective date */
function getEffectiveDate(row: MaRow): string {
  return row.maPeriodEnd?.trim() ?? "";
}

/** Team → zone key (same mapping as NS, extended for MA) */
...
function getAddress(row: MaRow): string {
...
export default function MaBaseReport() {
  const { GetMaReportData, downloadMAReportExcel, downloadMAReportPdf } =
    useApiConfig();

  // ── State ──────────────────────────────────────────────────────────────────
  const [data, setData] = useState<MaRow[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await GetMaReportData(startDate, lastDate);
  ...
  const handleExcelDownload = async () => {
    setDlLoading("excel");
    try {
      const blob: Blob = await downloadMAReportExcel(startDate, lastDate);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `MA_Report_${startDate}_to_${lastDate}.xlsx`;
      a.click();
  ...
  const handlePdfDownload = async () => {
    setDlLoading("pdf");
    try {
      const blob: Blob = await downloadMAReportPdf(startDate, lastDate);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `MA_Report_${startDate}_to_${lastDate}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e: any) {
  ...
  // ── Monthly counts for selected chart year ─────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, #2d1654 0%, #4a1d96 60%, #6d28d9 100%)",
          padding: "18px 28px",
          display: "flex",
      ...
      >
        <div>
          ...
          <div style={{ color: "#ddd6fe", fontSize: "11px", marginTop: "2px" }}>
            {loading
              ? "Loading…"
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    outline: active
                      ? `2px solid ${z === "ALL" ? "#4a1d96" : col.color}`
                      : "none",
                  }}
                >
        ...
        {(
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: "13px",
              color: activeTab === tab ? "#4a1d96" : "#94a3b8",
              borderBottom:
                activeTab === tab
                  ? "3px solid #4a1d96"
            ...
            <div style={{ overflowX: "auto" }}>
              ...
              >
                <thead>
                  <tr
                    style={{
                      background: "linear-gradient(135deg, #2d1654, #4a1d96)",
                    }}
                  >
                    {[
                      "#",
                ...
                {availableYears.map((y) => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontSize: "13px",
                      background: chartYear === y ? "#4a1d96" : "#f1f5f9",
              ...
              <div style={{ marginBottom: "4px" }}>
                ...
                >
                  {selectedMonth !== null
                    ...
                    : `Monthly MA Expiry Count — ${chartYear}`}
                </h3>
                <p
                  style={{
                    margin: "4px 0 20px",
                          ...
                          <div
                            ...
                            style={{
                              ...
                              background: isHot
                                ...
                                : count > 0
                                  ? "linear-gradient(180deg, #a78bfa 0%, #4a1d96 100%)"
                                  : "#f1f5f9",
                    ...
                    {MONTHS.map(
                      (m, i) =>
                        monthlyData[i] > 0 && (
                          ...
                          >
                            <span style={{ fontWeight: 700, color: "#4a1d96" }}>
                              {m}:
                            </span>{" "}
      ...
      >
        Effective date = MA Period End Date &nbsp;|&nbsp; Grade: A = Active · E
        = Expired · H = Hold
      </div>
    </div>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "6px",
        border: "1px solid #e2e8f0",
        background: active ? "#4a1d96" : "white",
